 ...
 */
package wtf.moonlight.util.waveycapes.renderlayers;

import net.minecraft.util.ResourceLocation;
import wtf.moonlight.Client;
import wtf.moonlight.module.impl.display.Interface;
import wtf.moonlight.util.render.shader.ShaderUtil;
import wtf.moonlight.util.waveycapes.CapeMovement;
import wtf.moonlight.util.waveycapes.CapeStyle;
import wtf.moonlight.util.waveycapes.WindMode;
import wtf.moonlight.util.waveycapes.config.Config;
import wtf.moonlight.util.waveycapes.sim.StickSimulation;
import wtf.moonlight.util.waveycapes.util.Mth;
...
public class CustomCapeRenderLayer implements LayerRenderer<AbstractClientPlayer> {
    ...
    private static final ShaderUtil cape = new ShaderUtil("mainmenu");
    ...
    public void doRenderLayer(AbstractClientPlayer abstractClientPlayer, float paramFloat1, float paramFloat2, float deltaTick,
        ...
        if (!Client.INSTANCE.getModuleManager().getModule(Interface.class).wavey.get())
            return;

        if (!abstractClientPlayer.isInvisible() && (abstractClientPlayer.hasPlayerInfo() && abstractClientPlayer.isWearing(EnumPlayerModelParts.CAPE) && abstractClientPlayer.getLocationCape() != null || abstractClientPlayer.getName().equals(Minecraft.getMinecraft().getSession().getUsername()) && Client.INSTANCE.getModuleManager().getModule(Interface.class).isEnabled() && Client.INSTANCE.getModuleManager().getModule(Interface.class).cape.get())) {
            if (Config.capeMovement == CapeMovement.BASIC_SIMULATION) {
                abstractClientPlayer.updateSimulation(abstractClientPlayer, partCount);
            }

            if (abstractClientPlayer == Minecraft.getMinecraft().thePlayer && Client.INSTANCE.getModuleManager().getModule(Interface.class).cape.get()) {
            ...
            if (Config.capeStyle == CapeStyle.SMOOTH) {
                if (abstractClientPlayer == Minecraft.getMinecraft().thePlayer) {
                    if (Client.INSTANCE.getModuleManager().getModule(Interface.class).cape.get()) {
