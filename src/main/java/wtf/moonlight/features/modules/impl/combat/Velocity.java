...
public class Velocity extends Module {
    ...
    private final ModeValue jumpResetMode = new ModeValue("Jump Reset Mode", new String[]{"Hurt Time", "Packet", "Advanced"}, "Packet", this, () -> mode.is("Jump Reset"));
    ...
    public void onStrafe(StrafeEvent event) {
        if (mode.is("Jump Reset")) {
            ...
            if (jumpResetMode.is("Packet") && veloPacket) {
                shouldJump = true;
            } else if (jumpResetMode.is("Hurt Time") && mc.thePlayer.hurtTime >= jumpResetHurtTime.get()) {
                shouldJump = true;
            } else if (jumpResetMode.is("Advanced")) {
                if (random.nextInt(100) > jumpResetChance.get()) return;

                boolean hitsCondition = hitsCount >= hitsUntilJump.get();
                boolean ticksCondition = ticksCount >= ticksUntilJump.get();
