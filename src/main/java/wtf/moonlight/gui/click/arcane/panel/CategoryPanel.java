...
public class CategoryPanel implements IComponent, InstanceAccess {
    ...
    public void drawScreen(int mouseX, int mouseY) {
        ...
        if (isSelected()) {
            ...
            RenderUtils.scissor(getPosX(), getPosY() + 75, INSTANCE.getArcaneClickGui().w, INSTANCE.getArcaneClickGui().h - 120);
    ...
    public void onScroll(int ms, int mx, int my) {
        ...
        if (RenderUtils.isHovering(getPosX(), getPosY() + 75, INSTANCE.getArcaneClickGui().w, INSTANCE.getArcaneClickGui().h - 120, mx, my) && moduleComponents.stream().noneMatch(moduleComponent -> moduleComponent.getComponents().stream().anyMatch(component -> component.isHovered(mx,my)))) {
