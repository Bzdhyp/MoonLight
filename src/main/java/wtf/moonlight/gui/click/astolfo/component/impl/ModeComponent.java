...
public class ModeComponent extends Component {
    ...
    public ModeComponent(ListValue value) {
        this.value = value;
        setHeight(11);
    }
    ...
    public void drawScreen(int mouseX, int mouseY) {
        ...
        Fonts.interSemiBold.get(13).drawString(value.getValue(),
                getX() + (getWidth() - 5) - Fonts.interSemiBold.get(13).getStringWidth(value.getValue()), getY() + 4F,
                -1);
        ...
        if (MouseUtils.isHovered2(getX(), getY(), 100F, getHeight(), mouseX, mouseY)) {
            if (mouseButton == 0) {
                ...
                for (int index = 0; index < value.getModes().length; index++) {
                    if (Objects.equals(value.getValue(), value.getModes()[index])) {
