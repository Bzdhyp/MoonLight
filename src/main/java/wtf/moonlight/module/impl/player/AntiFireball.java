            ...
            if (entity instanceof EntityFireball && entity.getDistanceToEntity(mc.thePlayer) < range.getValue() && isEnabled(LongJump.class)) {
                if (attackTimer.hasTimeElapsed((long) (1000L / (aps.getValue() + 2)))) {
                    ...
                    if (customRotationSetting.get()) {
                        RotationUtils.setRotation(finalRotation,smoothMode.getValue(), moveFix.get() ?  MovementCorrection.SILENT : MovementCorrection.OFF, minYawRotSpeed.getValue(), maxYawRotSpeed.getValue(), minPitchRotSpeed.getValue(), maxPitchRotSpeed.getValue(),
                                bezierP0.getValue(),
                                bezierP1.getValue(),
                                bezierP2.getValue(),
                                bezierP3.getValue(),
