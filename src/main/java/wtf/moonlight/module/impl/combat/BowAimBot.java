...
public class BowAimBot extends Module {
    ...
    public void onUpdate(UpdateEvent event) {
        ...
        if (target == null)
            return;
        if ((RotationUtils.getRotationDifference(target) <= fov.getValue() || fov.getValue() == 180) && mc.thePlayer.getHeldItem() != null && mc.thePlayer.getHeldItem().getItem() instanceof ItemBow && mc.thePlayer.isUsingItem()) {
            float[] finalRotation = RotationUtils.faceTrajectory(target, true, predictSize.getValue());
